...
RUN npm ci \
  ...
  && npm pack -w @coderloco/coderloco --pack-destination ./packages/cli/dist \
  && npm pack -w @coderloco/coderloco-core --pack-destination ./packages/core/dist

# Runtime stage
FROM docker.io/library/node:20-slim

ARG SANDBOX_NAME="coderloco-sandbox"
ARG CLI_VERSION_ARG
