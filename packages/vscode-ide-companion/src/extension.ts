...
import {
  ...
  IDE_DEFINITIONS,
  type IdeInfo,
} from '@coderloco/coderloco-core/src/ide/detect-ide.js';

const CLI_IDE_COMPANION_IDENTIFIER = 'locolm.loco-code-vscode-ide-companion';
const INFO_MESSAGE_SHOWN_KEY = 'locoCodeInfoMessageShown';
export const DIFF_SCHEME = 'loco-diff';
  ...
  try {
    ...
    if (latestVersion && semver.gt(latestVersion, currentVersion)) {
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the LOCO Code Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('LOCO Code Companion');
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('loco.diff.accept', (uri?: vscode.Uri) => {
      const docUri = uri ?? vscode.window.activeTextEditor?.document.uri;
      if (docUri && docUri.scheme === DIFF_SCHEME) {
        diffManager.acceptDiff(docUri);
      }
    }),
    vscode.commands.registerCommand('loco.diff.cancel', (uri?: vscode.Uri) => {
      const docUri = uri ?? vscode.window.activeTextEditor?.document.uri;
      if (docUri && docUri.scheme === DIFF_SCHEME) {
  ...
  if (!context.globalState.get(INFO_MESSAGE_SHOWN_KEY) && infoMessageEnabled) {
    void vscode.window.showInformationMessage(
      'LOCO Code Companion extension successfully installed.',
    );
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('loco-code.runlocoCode', async () => {
      const workspaceFolders = vscode.workspace.workspaceFolders;
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run LOCO Code.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run LOCO Code in',
        });
      }
      ...
      if (selectedFolder) {
        ...
        const terminal = vscode.window.createTerminal({
          name: `LOCO Code (${selectedFolder.name})`,
          cwd: selectedFolder.uri.fsPath,
        });
        terminal.show();
        terminal.sendText(locoCmd);
      }
    }),
    vscode.commands.registerCommand('loco-code.showNotices', async () => {
