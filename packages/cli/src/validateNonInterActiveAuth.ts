...
import { AuthType, OutputFormat } from '@coderloco/coderloco-core';
...
function getAuthTypeFromEnv(): AuthType | undefined {
  ...
  if (process.env['loco_OAUTH']) {
    return AuthType.loco_OAUTH;
  }
...
export async function validateNonInteractiveAuth(
  ...
  try {
    ...
    if (!effectiveAuthType) {
      const message = `Please set an Auth method in your ${USER_SETTINGS_PATH} or specify one of the following environment variables before running: loco_OAUTH, OPENAI_API_KEY`;
      throw new Error(message);
    }
