...
import { getProjectSummaryPrompt } from '@coderloco/coderloco-core';
import type { HistoryItemSummary } from '../types.js';

export const summaryCommand: SlashCommand = {
  name: 'summary',
  description:
    'Generate a project summary and save it to .loco/PROJECT_SUMMARY.md',
  ...
  action: async (context): Promise<SlashCommandActionReturn> => {
    ...
    try {
      ...
      const locoDir = path.join(projectRoot, '.loco');
      try {
        await fsPromises.mkdir(locoDir, { recursive: true });
      } catch (_err) {
        // Directory might already exist, ignore error
      }

      // Save the summary to PROJECT_SUMMARY.md
      const summaryPath = path.join(locoDir, 'PROJECT_SUMMARY.md');
...
`;
      ...
      const completedSummaryItem: HistoryItemSummary = {
        ...
        summary: {
          isPending: false,
          stage: 'completed',
          filePath: '.loco/PROJECT_SUMMARY.md',
        },
      };
