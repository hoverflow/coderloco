  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate.latest, currentVersion)) {
        const message = `A new version of LOCO Code is available! ${currentVersion} → ${bestUpdate.latest}`;
        return {
          message,
      ...
      if (updateInfo && semver.gt(updateInfo.latest, currentVersion)) {
        const message = `LOCO Code update available! ${currentVersion} → ${updateInfo.latest}`;
        return {
          message,
