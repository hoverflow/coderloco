...
interface LocoOAuthProgressProps {
  onTimeout: () => void;
  onCancel: () => void;
...
export function LocoOAuthProgress({
  onTimeout,
  onCancel,
  deviceAuth,
  authStatus,
  authMessage,
}: LocoOAuthProgressProps): React.JSX.Element {
    ...
    return (
      ...
      >
        <Text bold color={Colors.AccentRed}>
          LOCO OAuth Authentication Timeout
        </Text>
  ...
  if (!deviceAuth) {
    return (
      ...
      >
        <Box>
          <Text>
            <Spinner type="dots" /> Waiting for LOCO OAuth authentication...
          </Text>
        </Box>
        <Box marginTop={1} justifyContent="space-between">
          <Text color={Colors.Gray}>
