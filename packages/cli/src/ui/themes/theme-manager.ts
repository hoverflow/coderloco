...
import { locoLight } from './loco-light.js';
import { locoDark } from './loco-dark.js';
...
export const DEFAULT_THEME: Theme = locoDark;
...
class ThemeManager {
  ...
  getAvailableThemes(): ThemeDisplay[] {
    ...
    // Separate LOCO themes
    const locoThemes = builtInThemes.filter(
      (theme) => theme.name === locoLight.name || theme.name === locoDark.name,
    );
    ...
    // Combine LOCO themes first, then sorted others
    return [...locoThemes, ...sortedOtherThemes];
  }
