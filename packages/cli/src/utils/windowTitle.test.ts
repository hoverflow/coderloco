...
describe('computeWindowTitle', () => {
  ...
  it('should use default LOCO title when CLI_TITLE is not set', () => {
    const result = computeWindowTitle('my-project');
    expect(result).toBe('LOCO - my-project');
  });
  ...
  it('should handle folder names with special characters', () => {
    ...
    expect(result).toBe('LOCO - project-name_v1.0');
  });
});
