...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .locoignore patterns even when not in a git repo', async () => {
      await createTestFile('.locoignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);

      expect(service.shouldlocoIgnoreFile('secrets.txt')).toBe(true);
      expect(service.shouldlocoIgnoreFile('src/index.js')).toBe(false);
    });
  });
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.locoignore', 'logs/');
    });

    it('should filter out git-ignored and loco-ignored files by default', () => {
      const files = [
        'src/index.ts',
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectlocoIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectlocoIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('shouldGitIgnoreFile & shouldlocoIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.locoignore', '*.log');
    });
    ...
    it('should return true for loco-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldlocoIgnoreFile(path.join(projectRoot, 'debug.log')),
      ).toBe(true);
    });

    it('should return false for non-loco-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldlocoIgnoreFile(path.join(projectRoot, 'src/index.ts')),
      ).toBe(false);
    });
  });
