...
export class IdeClient {
  ...
  async connect(): Promise<void> {
    if (!this.currentIde) {
      this.setState(
        ...
        `IDE integration is not supported in your current environment. To use this feature, run LOCO Code in one of these supported IDEs: VS Code or VS Code forks`,
        false,
      );
      return;
    }
  ...
  static validateWorkspacePath(
    ...
    if (!isWithinWorkspace) {
      return {
        isValid: false,
        error: `Directory mismatch. LOCO Code is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
          ', ',
        )}`,
      };
    }
  ...
  private getPortFromEnv(): string | undefined {
    const port = process.env['loco_CODE_IDE_SERVER_PORT'];
    if (!port) {
      return undefined;
    }
    return port;
  ...
  private getStdioConfigFromEnv(): StdioConfig | undefined {
    const command = process.env['loco_CODE_IDE_SERVER_STDIO_COMMAND'];
    if (!command) {
      return undefined;
    }

    const argsStr = process.env['loco_CODE_IDE_SERVER_STDIO_ARGS'];
    let args: string[] = [];
    if (argsStr) {
      try {
        ...
        if (Array.isArray(parsedArgs)) {
          ...
          logger.error(
            'loco_CODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.',
          );
        }
      } catch (e) {
        logger.error('Failed to parse loco_CODE_IDE_SERVER_STDIO_ARGS:', e);
      }
    }
  ...
  > {
    ...
    try {
      const portFile = path.join(
        os.tmpdir(),
        `loco-code-ide-server-${this.ideProcessInfo.pid}.json`,
      );
      const portFileContents = await fs.promises.readFile(portFile, 'utf8');
      return JSON.parse(portFileContents);
    } catch (_) {
      // For newer extension versions, the file name matches the pattern
      // /^loco-code-ide-server-${pid}-\d+\.json$/. If multiple IDE
    ...
    const fileRegex = new RegExp(
      `^loco-code-ide-server-${this.ideProcessInfo.pid}-\\d+\\.json$`,
    );
