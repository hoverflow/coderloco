...
describe('IdeClient', () => {
  ...
  beforeEach(async () => {
    ...
    process.env['loco_CODE_IDE_WORKSPACE_PATH'] = '/test/workspace';
    delete process.env['loco_CODE_IDE_SERVER_PORT'];
    delete process.env['loco_CODE_IDE_SERVER_STDIO_COMMAND'];
    delete process.env['loco_CODE_IDE_SERVER_STDIO_ARGS'];
  ...
  describe('connect', () => {
    it('should connect using HTTP when port is provided in config file', async () => {
      ...
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp', 'loco-code-ide-server-12345.json'),
        'utf8',
      );
    ...
    it('should connect using HTTP when port is provided in environment variables', async () => {
      ...
      process.env['loco_CODE_IDE_SERVER_PORT'] = '9090';
    ...
    it('should connect using stdio when stdio config is in environment variables', async () => {
      ...
      process.env['loco_CODE_IDE_SERVER_STDIO_COMMAND'] = 'env-cmd';
      process.env['loco_CODE_IDE_SERVER_STDIO_ARGS'] = '["--bar"]';
  ...
  describe('getConnectionConfigFromFile', () => {
    ...
    it('should find and parse a single config file with the new naming scheme', async () => {
      ...
      (
        vi.mocked(fs.promises.readdir) as Mock<
          (path: fs.PathLike) => Promise<string[]>
        >
      ).mockResolvedValue(['loco-code-ide-server-12345-123.json']);
      vi.mocked(fs.promises.readFile).mockResolvedValue(JSON.stringify(config));
      ...
      expect(result).toEqual(config);
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp/gemini/ide', 'loco-code-ide-server-12345-123.json'),
        'utf8',
      );
    });
    ...
    it('should filter out configs with invalid workspace paths', async () => {
      ...
      (
        ...
        'loco-code-ide-server-12345-111.json',
        'loco-code-ide-server-12345-222.json',
      ]);
    ...
    it('should prioritize the config matching the port from the environment variable', async () => {
      process.env['loco_CODE_IDE_SERVER_PORT'] = '2222';
      const config1 = { port: '1111', workspacePath: '/test/workspace' };
      const config2 = { port: '2222', workspacePath: '/test/workspace2' };
      vi.mocked(fs.promises.readFile).mockRejectedValueOnce(
        new Error('not found'),
    ...
    it('should ignore files with invalid names', async () => {
      ...
      (
        ...
        'loco-code-ide-server-12345-111.json', // valid
        'not-a-config-file.txt', // invalid
        'loco-code-ide-server-asdf.json', // invalid
      ]);
      ...
      expect(result).toEqual(validConfig);
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp/gemini/ide', 'loco-code-ide-server-12345-111.json'),
        'utf8',
      );
    ...
    it('should match env port string to a number port in the config', async () => {
      process.env['loco_CODE_IDE_SERVER_PORT'] = '3333';
      const config1 = { port: 1111, workspacePath: '/test/workspace' };
