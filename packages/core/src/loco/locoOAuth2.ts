...
// OAuth Endpoints
const loco_OAUTH_BASE_URL = 'https://chat.loco.ai';

const loco_OAUTH_DEVICE_CODE_ENDPOINT = `${loco_OAUTH_BASE_URL}/api/v1/oauth2/device/code`;
const loco_OAUTH_TOKEN_ENDPOINT = `${loco_OAUTH_BASE_URL}/api/v1/oauth2/token`;

// OAuth Client Configuration
const loco_OAUTH_CLIENT_ID = 'f0304373b74a44d2b584a3fb70ca9e56';

const loco_OAUTH_SCOPE = 'openid profile email model.completion';
const loco_OAUTH_GRANT_TYPE = 'urn:ietf:params:oauth:grant-type:device_code';
...
/**
 * LOCO OAuth2 credentials interface
 */
...
/**
 * LOCO OAuth2 client interface
 */
export interface IlocoOAuth2Client {
  setCredentials(credentials: locoCredentials): void;
  getCredentials(): locoCredentials;
...
/**
 * LOCO OAuth2 client implementation
 */
export class locoOAuth2Client implements IlocoOAuth2Client {
  private credentials: locoCredentials = {};
  ...
  async requestDeviceAuthorization(options: {
    ...
    const bodyData = {
      client_id: loco_OAUTH_CLIENT_ID,
      scope: options.scope,
      code_challenge: options.code_challenge,
      code_challenge_method: options.code_challenge_method,
    };

    const response = await fetch(loco_OAUTH_DEVICE_CODE_ENDPOINT, {
      method: 'POST',
      headers: {
  ...
  async pollDeviceToken(options: {
    ...
    const bodyData = {
      grant_type: loco_OAUTH_GRANT_TYPE,
      client_id: loco_OAUTH_CLIENT_ID,
      device_code: options.device_code,
      code_verifier: options.code_verifier,
    };

    const response = await fetch(loco_OAUTH_TOKEN_ENDPOINT, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        Accept: 'application/json',
...
/**
 * Global event emitter instance for locoOAuth2 authentication events
 */
export const locoOAuth2Events = new EventEmitter();

export async function getlocoOAuthClient(
  config: Config,
): Promise<locoOAuth2Client> {
  ...
  try {
    ...
    if (await loadCachedlocoCredentials(client)) {
      // We have cached credentials but they might be expired
      // Try device flow instead of forcing refresh
      const result = await authWithlocoDeviceFlow(client, config);
      if (!result.success) {
        throw new Error('LOCO OAuth authentication failed');
      }
      return client;
    }
    ...
    if (!result.success) {
      ...
      switch (result.reason) {
        case 'timeout':
          throw new Error('LOCO OAuth authentication timed out');
        case 'cancelled':
          throw new Error('LOCO OAuth authentication was cancelled by user');
        case 'rate_limit':
          throw new Error(
...
async function authWithlocoDeviceFlow(
  client: locoOAuth2Client,
  config: Config,
): Promise<AuthResult> {
  ...
  locoOAuth2Events.once(locoOAuth2Event.AuthCancel, cancelHandler);
  ...
  try {
    ...
    locoOAuth2Events.emit(locoOAuth2Event.AuthUri, deviceAuth);

    const showFallbackMessage = () => {
      console.log('\n=== LOCO OAuth Device Authorization ===');
      console.log(
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (isDeviceTokenSuccess(tokenResponse)) {
          ...
          // Convert to locoCredentials format
          ...
          await cachelocoCredentials(credentials);
        ...
        if (errorMessage.includes('401') || statusCode === 401) {
          ...
          // Emit error event
          locoOAuth2Events.emit(locoOAuth2Event.AuthProgress, 'error', message);
    ...
    locoOAuth2Events.off(locoOAuth2Event.AuthCancel, cancelHandler);
  }
}

async function loadCachedlocoCredentials(
  client: locoOAuth2Client,
): Promise<boolean> {
  try {
    const keyFile = getlocoCachedCredentialPath();
    const creds = await fs.readFile(keyFile, 'utf-8');
    const credentials = JSON.parse(creds) as locoCredentials;
    client.setCredentials(credentials);
...
async function cachelocoCredentials(credentials: locoCredentials) {
  const filePath = getlocoCachedCredentialPath();
...
/**
 * Clear cached LOCO credentials from disk
 * This is useful when credentials have expired or need to be reset
 */
export async function clearlocoCredentials(): Promise<void> {
  try {
    const filePath = getlocoCachedCredentialPath();
    await fs.unlink(filePath);
    console.debug('Cached LOCO credentials cleared successfully.');
  } catch (error: unknown) {
    ...
    console.warn('Warning: Failed to clear cached LOCO credentials:', error);
  }
}
