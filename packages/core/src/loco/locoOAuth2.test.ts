...
interface MockSharedTokenManager {
  getValidCredentials(locoClient: locoOAuth2Client): Promise<locoCredentials>;
  getCurrentCredentials(): locoCredentials | null;
  clearCache(): void;
}
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    async getValidCredentials(
      locoClient: locoOAuth2Client,
    ): Promise<locoCredentials> {
      // Try to get credentials from the client first
      const clientCredentials = locoClient.getCredentials();
  ...
  describe('isErrorResponse', () => {
    ...
    it('should return false for successful responses', () => {
      const successResponse: DeviceAuthorizationResponse = {
        device_code: 'test-device-code',
        user_code: 'TEST123',
        verification_uri: 'https://chat.loco.ai/device',
        verification_uri_complete: 'https://chat.loco.ai/device?code=TEST123',
        expires_in: 1800,
      };
...
describe('locoOAuth2Client', () => {
  let client: locoOAuth2Client;
  let originalFetch: typeof global.fetch;

  beforeEach(() => {
    // Create client instance
    client = new locoOAuth2Client();
    ...
    it('should refresh token if access token is expired', async () => {
      ...
      (
        client as unknown as {
          sharedManager: {
            getValidCredentials: () => Promise<locoCredentials>;
          };
        }
      ).sharedManager = {
...
describe('getlocoOAuthClient', () => {
  ...
  it('should load cached credentials if available', async () => {
    ...
    const client = await import('./locoOAuth2.js').then((module) =>
      module.getlocoOAuthClient(mockConfig),
    );
  ...
  it('should handle cached credentials refresh failure', async () => {
    ...
    const mockCredentials = {
      ...
      token_type: 'Bearer',
      expiry_date: Date.now() + 3600000, // Valid expiry time so loadCachedlocoCredentials returns true
    };
    ...
    await expect(
      import('./locoOAuth2.js').then((module) =>
        module.getlocoOAuthClient(mockConfig),
      ),
    ).rejects.toThrow('LOCO OAuth authentication failed');
...
describe('CredentialsClearRequiredError', () => {
  it('should create error with correct name and message', async () => {
    const { CredentialsClearRequiredError } = await import('./locoOAuth2.js');
...
describe('clearlocoCredentials', () => {
  it('should successfully clear credentials file', async () => {
    const { promises: fs } = await import('node:fs');
    const { clearlocoCredentials } = await import('./locoOAuth2.js');

    vi.mocked(fs.unlink).mockResolvedValue(undefined);

    await expect(clearlocoCredentials()).resolves.not.toThrow();
...
describe('locoOAuth2Client - Additional Error Scenarios', () => {
  let client: locoOAuth2Client;
  let originalFetch: typeof global.fetch;
...
describe('getlocoOAuthClient - Enhanced Error Scenarios', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
  ...
  it('should handle different authentication failure reasons - timeout', async () => {
    ...
    ).rejects.toThrow('LOCO OAuth authentication timed out');
  ...
  it('should handle authentication failure reason - rate limit', async () => {
    ...
    await expect(
      ...
      'Too many request for LOCO OAuth authentication, please try again later.',
    );
...
describe('authWithlocoDeviceFlow - Comprehensive Testing', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
...
describe('Event Emitter Integration', () => {
  it('should export locoOAuth2Events as EventEmitter', async () => {
    const { locoOAuth2Events } = await import('./locoOAuth2.js');
    expect(locoOAuth2Events).toBeInstanceOf(EventEmitter);
  });

  it('should define correct event enum values', async () => {
    const { locoOAuth2Event } = await import('./locoOAuth2.js');
    expect(locoOAuth2Event.AuthUri).toBe('auth-uri');
    expect(locoOAuth2Event.AuthProgress).toBe('auth-progress');
    expect(locoOAuth2Event.AuthCancel).toBe('auth-cancel');
  });
});
...
describe('Utility Functions', () => {
  ...
  describe('getlocoCachedCredentialPath', () => {
    it('should return correct path to cached credentials', async () => {
      const os = await import('os');
      const path = await import('path');

      const expectedPath = path.join(os.homedir(), '.loco', 'oauth_creds.json');

      // Since this is a private function, we test it indirectly through clearlocoCredentials
      const { promises: fs } = await import('node:fs');
      const { clearlocoCredentials } = await import('./locoOAuth2.js');
...
describe('Credential Caching Functions', () => {
  describe('cachelocoCredentials', () => {
    it('should create directory and write credentials to file', async () => {
      // Mock the internal cachelocoCredentials function by creating client and calling refresh
      const client = new locoOAuth2Client();
      client.setCredentials({
        refresh_token: 'test-refresh',
      });
  ...
  describe('loadCachedlocoCredentials', () => {
    it('should load and validate cached credentials successfully', async () => {
      ...
      // Test through getlocoOAuthClient which calls loadCachedlocoCredentials
      const mockConfig = {
      ...
      try {
        await import('./locoOAuth2.js').then((module) =>
          module.getlocoOAuthClient(mockConfig),
        );
      } catch {
...
describe('Enhanced Error Handling and Edge Cases', () => {
  ...
  describe('locoOAuth2Client getAccessToken enhanced scenarios', () => {
  ...
  describe('Enhanced refreshAccessToken scenarios', () => {
    it('should call clearlocoCredentials on 400 error', async () => {
      client.setCredentials({
        refresh_token: 'expired-refresh',
      });
...
describe('SharedTokenManager Integration in locoOAuth2Client', () => {
  let client: locoOAuth2Client;
  ...
  it('should handle TokenManagerError types correctly in getlocoOAuthClient', async () => {
    const mockConfig = {
      isBrowserLaunchSuppressed: vi.fn().mockReturnValue(true),
    } as unknown as Config;
...
describe('Constants and Configuration', () => {
  it('should have correct OAuth endpoints', async () => {
    ...
    expect(url).toBe('https://chat.loco.ai/api/v1/oauth2/device/code');
  });
