...
const loco_LOCK_FILENAME = 'oauth_creds.lock';
...
interface MemoryCache {
  credentials: locoCredentials | null;
  fileModTime: number;
  lastCheck: number;
}

/**
 * Validates that the given data is a valid locoCredentials object
 *
 * @param data - The data to validate
 * @returns The validated credentials object
 * @throws Error if the data is invalid
 */
function validateCredentials(data: unknown): locoCredentials {
  if (!data || typeof data !== 'object') {
    throw new Error('Invalid credentials format');
  }

  const creds = data as Partial<locoCredentials>;
  ...
  return creds as locoCredentials;
}
...
export class SharedTokenManager {
   ...
   */
  private refreshPromise: Promise<locoCredentials> | null = null;
   ...
   *
   * @param locoClient - The OAuth2 client instance
  ...
  async getValidCredentials(
    ...
    try {
      ...
      await this.checkAndReloadIfNeeded(locoClient);
  ...
  private async checkAndReloadIfNeeded(
    ...
    this.checkPromise = this.performFileCheck(locoClient, now);
  ...
  private async performFileCheck(
    locoClient: IlocoOAuth2Client | undefined,
    checkTime: number,
  ): Promise<void> {
    ...
    try {
      ...
      if (fileModTime > this.memoryCache.fileModTime) {
        await this.reloadCredentialsFromFile(locoClient);
   ...
   */
  private async forceFileCheck(locoClient?: IlocoOAuth2Client): Promise<void> {
    try {
      const filePath = this.getCredentialFilePath();
  ...
  /**
   * Load credentials from the file system into memory cache and sync with locoClient
   */
  private async reloadCredentialsFromFile(
    ...
    try {
      ...
      // Sync with locoClient atomically - rollback on failure
      try {
        if (locoClient) {
          locoClient.setCredentials(credentials);
        }
      } catch (clientError) {
  ...
  private async performTokenRefresh(
    ...
    try {
      ...
      const currentCredentials = locoClient.getCredentials();
      ...
      await this.forceFileCheck(locoClient);
      ...
      if (
        ...
        // No need to call locoClient.setCredentials here as checkAndReloadIfNeeded already did it
        return this.memoryCache.credentials;
      }

      // Perform the actual token refresh
      const response = await locoClient.refreshAccessToken();
   ...
   */
  private isTokenValid(credentials: locoCredentials): boolean {
  ...
  private getLockFilePath(): string {
    return path.join(os.homedir(), loco_DIR, loco_LOCK_FILENAME);
  }
