...
import { locoLogger, TEST_ONLY } from './loco-logger.js';
...
const makeFakeConfig = (overrides: Partial<Config> = {}): Config => {
  const defaults = {
    ...
    getAuthType: () => AuthType.loco_OAUTH,
    getMcpServers: () => ({}),
    getModel: () => 'test-model',
...
describe('locoLogger', () => {
  ...
  beforeEach(() => {
    ...
    (locoLogger as any).instance = undefined;
  });
  ...
  describe('getInstance', () => {
    it('returns undefined when usage statistics are disabled', () => {
      ...
      const logger = locoLogger.getInstance(config);
      expect(logger).toBeUndefined();
    });

    it('returns an instance when usage statistics are enabled', () => {
      const logger = locoLogger.getInstance(mockConfig);
      expect(logger).toBeInstanceOf(locoLogger);
    });

    it('is a singleton', () => {
      const logger1 = locoLogger.getInstance(mockConfig);
      const logger2 = locoLogger.getInstance(mockConfig);
  ...
  describe('event queue management', () => {
    it('should handle event overflow gracefully', () => {
      ...
      const logger = locoLogger.getInstance(debugConfig)!;
      const consoleSpy = vi
        .spyOn(console, 'debug')
        .mockImplementation(() => {});
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'locoLogger: Dropped old event to prevent memory leak',
        ),
      );
    });
    ...
    it('should handle enqueue errors gracefully', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'locoLogger: Failed to enqueue log event.',
        expect.any(Error),
      );
  ...
  describe('failed event retry mechanism', () => {
    it('should requeue failed events with size limits', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('locoLogger: Re-queued'),
      );
    });
    ...
    it('should handle empty retry queue gracefully', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('locoLogger: No events re-queued'),
      );
    });
  });
