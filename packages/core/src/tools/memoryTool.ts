  ...
  parametersJsonSchema: {
    ...
    properties: {
      ...
      scope: {
        type: 'string',
        description:
          'Where to save the memory: "global" saves to user-level ~/.loco/LOCO.md (shared across all projects), "project" saves to current project\'s LOCO.md (project-specific). If not specified, will prompt user to choose.',
        enum: ['global', 'project'],
      },
    },
    required: ['fact'],
...
- \`scope\` (string, optional): Where to save the memory:
  - "global": Saves to user-level ~/.loco/LOCO.md (shared across all projects)
  - "project": Saves to current project's LOCO.md (project-specific)
  - If not specified, the tool will ask the user where they want to save the memory.
`;

export const loco_CONFIG_DIR = '.loco';
export const DEFAULT_CONTEXT_FILENAME = 'LOCO.md';
export const MEMORY_SECTION_HEADER = '## LOCO Added Memories';

// This variable will hold the currently configured filename for LOCO.md context files.
...
function getGlobalMemoryFilePath(): string {
  return path.join(Storage.getGloballocoDir(), getCurrentGeminiMdFilename());
}
