...
export interface ReadManyFilesParams {
  ...
  /**
   * Whether to respect .gitignore and .locoignore patterns (optional, defaults to true)
   */
  file_filtering_options?: {
    respect_git_ignore?: boolean;
    respect_loco_ignore?: boolean;
  };
}
 ...
 * This combines the shared patterns with dynamic patterns like LOCO.md.
...
${this.config.getTargetDir()}
    ...
    const locoIgnorePatterns = this.config
      .getFileService()
      .getlocoIgnorePatterns();
    const finalExclusionPatternsForDescription: string[] =
      paramUseDefaultExcludes
        ...
        : [...paramExcludes, ...locoIgnorePatterns];
...
${finalExclusionPatternsForDescription
  ...
  .join(
    ...
    // Add a note if .locoignore patterns contributed to the final list of exclusions
    if (locoIgnorePatterns.length > 0) {
      const geminiPatternsInEffect = locoIgnorePatterns.filter((p) =>
        finalExclusionPatternsForDescription.includes(p),
      ).length;
      if (geminiPatternsInEffect > 0) {
        excludeDesc += ` (includes ${geminiPatternsInEffect} from .locoignore)`;
      }
    }
  ...
  async execute(signal: AbortSignal): Promise<ToolResult> {
    ...
    try {
      ...
      const { filteredPaths, gitIgnoredCount, locoIgnoredCount } =
        fileDiscovery.filterFilesWithReport(relativeEntries, {
          ...
          respectlocoIgnore:
            this.params.file_filtering_options?.respect_loco_ignore ??
            this.config.getFileFilteringOptions().respectlocoIgnore ??
            DEFAULT_FILE_FILTERING_OPTIONS.respectlocoIgnore,
        });
      ...
      // Add info about loco-ignored files if any were filtered
      if (locoIgnoredCount > 0) {
        skippedFiles.push({
          path: `${locoIgnoredCount} file(s)`,
          reason: 'LOCO ignored',
        });
      }
    } catch (error) {
  ...
  constructor(private config: Config) {
    const parameterSchema = {
      ...
      properties: {
        ...
        file_filtering_options: {
          description:
            'Whether to respect ignore patterns from .gitignore or .locoignore',
          type: 'object',
          properties: {
            respect_git_ignore: {
              description:
            ...
            respect_loco_ignore: {
              description:
                'Optional: Whether to respect .locoignore patterns when listing files. Defaults to true.',
              type: 'boolean',
            },
          },
        },
