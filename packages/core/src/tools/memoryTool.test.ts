...
const MEMORY_SECTION_HEADER = '## LOCO Added Memories';
...
describe('MemoryTool', () => {
  ...
  describe('performAddMemoryEntry (static method)', () => {
    ...
    beforeEach(() => {
      testFilePath = path.join(os.homedir(), '.loco', DEFAULT_CONTEXT_FILENAME);
    });
  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted for global scope', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', '.loco', 'LOCO.md');
        expect(result.title).toBe(
          `Confirm Memory Save: ${expectedPath} (global)`,
        );
        expect(result.fileName).toContain(path.join('mock', 'home', '.loco'));
        expect(result.fileName).toContain('LOCO.md');
        expect(result.fileDiff).toContain('Index: LOCO.md');
        expect(result.fileDiff).toContain('+## LOCO Added Memories');
        expect(result.fileDiff).toContain('+- Test fact');
        expect(result.originalContent).toBe('');
        expect(result.newContent).toContain('## LOCO Added Memories');
    ...
    it('should return confirmation details when memory file is not allowlisted for project scope', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join(process.cwd(), 'LOCO.md');
        expect(result.title).toBe(
    ...
    it('should prompt for scope selection when scope is not specified', async () => {
      ...
      if (result && result.type === 'edit') {
        ...
        expect(result.fileName).toBe('LOCO.md');
        expect(result.fileDiff).toContain('Test fact');
        expect(result.fileDiff).toContain('--- LOCO.md');
        expect(result.fileDiff).toContain('+++ LOCO.md');
        expect(result.fileDiff).toContain('+- Test fact');
    ...
    it('should show correct file paths in scope selection prompt', async () => {
      ...
      if (result && result.type === 'edit') {
        const globalPath = path.join('~', '.loco', 'LOCO.md');
        const projectPath = path.join(process.cwd(), 'LOCO.md');
