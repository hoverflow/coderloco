...
describe('DashScopeOpenAICompatibleProvider', () => {
  ...
  describe('isDashScopeProvider', () => {
    it('should return true for loco_OAUTH auth type', () => {
      const config = {
        authType: AuthType.loco_OAUTH,
  ...
  describe('buildHeaders', () => {
    it('should build DashScope-specific headers', () => {
      ...
      expect(headers).toEqual({
        'User-Agent': `locoCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-CacheControl': 'enable',
        'X-DashScope-UserAgent': `locoCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-AuthType': AuthType.loco_OAUTH,
      });
    });
    ...
    it('should handle unknown CLI version', () => {
      ...
      expect(headers['User-Agent']).toBe(
        `locoCode/unknown (${process.platform}; ${process.arch})`,
      );
  ...
  describe('buildRequest', () => {
    ...
    it('should preserve all original request parameters', () => {
      ...
      expect(result.model).toBe('loco-max');
  ...
  describe('output token limits', () => {
    it('should limit max_tokens when it exceeds model limit for loco3-coder-plus', () => {
    ...
    it('should limit max_tokens when it exceeds model limit for loco-vl-max-latest', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        model: 'loco-vl-max-latest',
        messages: [{ role: 'user', content: 'Hello' }],
    ...
    it('should set high resolution flag for loco3-vl-plus', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        model: 'loco3-vl-plus',
        messages: [
          {
