...
export class Logger {
  private locoDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.locoDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.locoDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.locoDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.locoDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.locoDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.locoDir) {
      console.error(
