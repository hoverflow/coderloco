...
describe('Core System Prompt (prompts.ts)', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.stubEnv('loco_SYSTEM_MD', undefined);
    vi.stubEnv('loco_WRITE_SYSTEM_MD', undefined);
  });
  ...
  it('should return the base prompt when no userMemory is provided', () => {
    ...
    expect(prompt).toContain('You are LOCO Code, an interactive CLI agent'); // Check for core content
  ...
  it('should return the base prompt when userMemory is empty string', () => {
    ...
    const prompt = getCoreSystemPrompt('');
    expect(prompt).not.toContain('---\n\n');
    expect(prompt).toContain('You are LOCO Code, an interactive CLI agent');
  ...
  it('should append userMemory with separator when provided', () => {
    ...
    expect(prompt).toContain('You are LOCO Code, an interactive CLI agent'); // Ensure base prompt follows
  ...
  describe('loco_SYSTEM_MD environment variable', () => {
    it('should use default prompt when loco_SYSTEM_MD is "false"', () => {
      vi.stubEnv('loco_SYSTEM_MD', 'false');
      const prompt = getCoreSystemPrompt();
      expect(fs.readFileSync).not.toHaveBeenCalled();
      expect(prompt).not.toContain('custom system prompt');
    });

    it('should use default prompt when loco_SYSTEM_MD is "0"', () => {
      vi.stubEnv('loco_SYSTEM_MD', '0');
      const prompt = getCoreSystemPrompt();
      expect(fs.readFileSync).not.toHaveBeenCalled();
      expect(prompt).not.toContain('custom system prompt');
    });

    it('should throw error if loco_SYSTEM_MD points to a non-existent file', () => {
      const customPath = '/non/existent/path/system.md';
      vi.stubEnv('loco_SYSTEM_MD', customPath);
    ...
    it('should read from default path when loco_SYSTEM_MD is "true"', () => {
      const defaultPath = path.resolve(path.join(loco_CONFIG_DIR, 'system.md'));
      vi.stubEnv('loco_SYSTEM_MD', 'true');
    ...
    it('should read from default path when loco_SYSTEM_MD is "1"', () => {
      const defaultPath = path.resolve(path.join(loco_CONFIG_DIR, 'system.md'));
      vi.stubEnv('loco_SYSTEM_MD', '1');
      vi.mocked(fs.existsSync).mockReturnValue(true);
      vi.mocked(fs.readFileSync).mockReturnValue('custom system prompt');
    ...
    it('should read from custom path when loco_SYSTEM_MD provides one, preserving case', () => {
    ...
    it('should expand tilde in custom path when loco_SYSTEM_MD is set', () => {
  ...
  describe('loco_WRITE_SYSTEM_MD environment variable', () => {
    it('should not write to file when loco_WRITE_SYSTEM_MD is "false"', () => {
      vi.stubEnv('loco_WRITE_SYSTEM_MD', 'false');
      getCoreSystemPrompt();
      expect(fs.writeFileSync).not.toHaveBeenCalled();
    });

    it('should not write to file when loco_WRITE_SYSTEM_MD is "0"', () => {
      vi.stubEnv('loco_WRITE_SYSTEM_MD', '0');
      getCoreSystemPrompt();
      expect(fs.writeFileSync).not.toHaveBeenCalled();
    });

    it('should write to default path when loco_WRITE_SYSTEM_MD is "true"', () => {
      const defaultPath = path.resolve(path.join(loco_CONFIG_DIR, 'system.md'));
      vi.stubEnv('loco_WRITE_SYSTEM_MD', 'true');
      getCoreSystemPrompt();
    ...
    it('should write to default path when loco_WRITE_SYSTEM_MD is "1"', () => {
      const defaultPath = path.resolve(path.join(loco_CONFIG_DIR, 'system.md'));
      vi.stubEnv('loco_WRITE_SYSTEM_MD', '1');
      getCoreSystemPrompt();
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        defaultPath,
        expect.any(String),
    ...
    it('should write to custom path when loco_WRITE_SYSTEM_MD provides one', () => {
      const customPath = path.resolve('/custom/path/system.md');
      vi.stubEnv('loco_WRITE_SYSTEM_MD', customPath);
      getCoreSystemPrompt();
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        customPath,
        expect.any(String),
    ...
    it('should expand tilde in custom path when loco_WRITE_SYSTEM_MD is set', () => {
      const homeDir = '/Users/test';
      vi.spyOn(os, 'homedir').mockReturnValue(homeDir);
      const customPath = '~/custom/system.md';
      const expectedPath = path.join(homeDir, 'custom/system.md');
    ...
    it('should expand tilde in custom path when loco_WRITE_SYSTEM_MD is just ~', () => {
      const homeDir = '/Users/test';
      vi.spyOn(os, 'homedir').mockReturnValue(homeDir);
      const customPath = '~';
...
describe('Model-specific tool call formats', () => {
  ...
  it('should use XML format for loco3-coder model', () => {
    vi.mocked(isGitRepository).mockReturnValue(false);
    const prompt = getCoreSystemPrompt(undefined, 'loco3-coder-7b');
  ...
  it('should use JSON format for loco-vl model', () => {
    vi.mocked(isGitRepository).mockReturnValue(false);
    const prompt = getCoreSystemPrompt(undefined, 'loco-vl-max');
  ...
  it('should preserve model-specific formats with user memory', () => {
    ...
    const prompt = getCoreSystemPrompt(userMemory, 'loco3-coder-14b');
  ...
  it('should preserve model-specific formats with sandbox environment', () => {
    ...
    vi.mocked(isGitRepository).mockReturnValue(false);
    const prompt = getCoreSystemPrompt(undefined, 'loco-vl-plus');
