...
async function getGeminiMdFilePathsInternalForEachDir(
  ...
  for (const geminiMdFilename of geminiMdFilenames) {
    ...
    if (isHomeDirectory) {
      // For home directory, only check for LOCO.md directly in the home directory
      ...
      while (currentDir && currentDir !== path.dirname(currentDir)) {
        if (currentDir === path.join(resolvedHome, loco_DIR)) {
          break;
        }
...
/**
 * Loads hierarchical LOCO.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  if (filePaths.length === 0) {
    if (debugMode) logger.debug('No LOCO.md files found in hierarchy.');
