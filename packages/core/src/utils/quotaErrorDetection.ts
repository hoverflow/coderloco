...
export function islocoQuotaExceededError(error: unknown): boolean {
  // Check for LOCO insufficient quota errors (should not retry)
  const checkMessage = (message: string): boolean => {
...
export function islocoThrottlingError(error: unknown): boolean {
  // Check for LOCO throttling errors (should retry)
  const checkMessage = (message: string): boolean => {
    const lowerMessage = message.toLowerCase();
    return (
