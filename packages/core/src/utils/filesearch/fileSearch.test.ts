...
import { createTmpDir, cleanupTmpDir } from '@coderloco/coderloco-test-utils';
...
describe('FileSearch', () => {
  ...
  it('should use .locoignore rules', async () => {
    ...
    expect(results).toEqual(['src/', '.locoignore', 'src/not-ignored.js']);
  });

  it('should combine .gitignore and .locoignore rules', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': 'dist/',
      '.locoignore': 'build/',
