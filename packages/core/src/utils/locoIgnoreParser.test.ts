...
import { locoIgnoreParser } from './locoIgnoreParser.js';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import * as os from 'node:os';

describe('locoIgnoreParser', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'locoignore-test-'));
  });
  ...
  describe('when .locoignore exists', () => {
    beforeEach(async () => {
    ...
    it('should ignore files specified in .locoignore', () => {
      const parser = new locoIgnoreParser(projectRoot);
  ...
  describe('when .locoignore does not exist', () => {
