...
function getVersionFromNPM(distTag) {
  const command = `npm view @loco-code/loco-code version --tag=${distTag}`;
  try {
...
function getAllVersionsFromNPM() {
  const command = `npm view @loco-code/loco-code versions --json`;
  try {
...
function isVersionDeprecated(version) {
  const command = `npm view @loco-code/loco-code@${version} deprecated`;
  try {
...
function doesVersionExist(version) {
  // Check NPM
  try {
    const command = `npm view @loco-code/loco-code@${version} version 2>/dev/null`;
    const output = execSync(command).toString().trim();
    if (output === version) {
