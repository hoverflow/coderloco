...
const USER_GEMINI_DIR = path.join(os.homedir(), '.loco');
// Project-level .gemini directory in the workspace
const WORKSPACE_GEMINI_DIR = path.join(projectRoot, '.loco');

// Telemetry artifacts are stored in a hashed directory under the user's ~/.loco/tmp
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `loco-code-releases-${Date.now()}.json`,
  );
  try {
    const result = spawnSync(
      'curl',
      ['-sL', '-H', 'User-Agent: loco-code-dev-script', '-o', tmpFile, url],
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'loco-code-telemetry-'));
